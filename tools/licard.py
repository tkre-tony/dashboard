...
CANONICAL RENDERER. Do NOT fork this per article. build() already takes every
per-drop value as an argument, so a numbered copy differs only in this comment.
Forking is how licard_224.py and licard_225.py were lost to 404, and how the
card legibility fixes sat uncommitted for two sessions.

    import licard
    licard.build(headline=..., kicker=..., stat_pairs=[(value, label), ...],
                 credit=..., out="li_card_NNN.jpg", photo="images/hero.jpg")

Legibility settings applied S342: credit 15px, stat labels 16px.
Credit line must name something visible ON the card (L-SOCIAL-12).
No flag emoji anywhere in accompanying post copy (L-SOCIAL-15).

COMMIT THIS FILE TO tools/licard.py.
...
def build(headline, kicker, stat_pairs, credit, out, photo=None):
    ...
    for i, (val, lab) in enumerate(stat_pairs):
        ...
        d.text((x, y + 44), lab, font=font(SANS, 16), fill=(0xB8, 0xC6, 0xDB))

    # credit line, L-SOCIAL-2, inside the L-IMG-1 margin floor
    fc = font(SANS, 15)
    lines = textwrap.wrap(credit, width=102)[:2]
    cy = H - MARGIN - (len(lines) * 20) + 4
    for ln in lines:
